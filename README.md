...
Three English-language audio Quick Actions for macOS Finder, installed entirely from Terminal:
...
All three live under **right-click → Quick Actions** and use macOS's built-in **Audio Output** speaker image (`NSTouchBarAudioOutputVolumeHigh`). Dialogs use the same image. Apple artwork is obtained from the local OS during installation, never bundled in this repository.
...
2. Exports the stock system speaker image and generates three Automator workflow bundles.
...
4. Enables only these three services in the current user's `pbs` service preferences and asks macOS to refresh service registration.
...
The core supports headless use with `FINDER_AUDIO_NONINTERACTIVE=1` and a nonzero exit status if any file fails. Custom mode additionally requires `--settings-json`, for example:
